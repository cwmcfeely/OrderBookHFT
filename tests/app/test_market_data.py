...
class TestMarketData(unittest.TestCase):
    def setUp(self):
        ...
        self.lock_patch = patch('app.market_data.threading.Lock', return_value=MagicMock(spec=threading.Lock))
        ...
        patcher_data_dir = patch('app.market_data.DATA_DIR_RAW', new_callable=lambda: Path(os.path.abspath(self.temp_dir)))
        patcher_api_count = patch('app.market_data.API_COUNT_FILE', new_callable=lambda: Path(os.path.join(self.temp_dir, "api_calls_today.json")))
    ...
    def test_increment_api_count_resets_on_new_day(self):
        ...
        yesterday = app.market_data.date.today().replace(day=max(1, app.market_data.date.today().day - 1))
    ...
    def test_fetch_intraday_data_api_and_cache(self, mock_cache, mock_fetch):
        ...
        symbol = "TEST"
        mock_fetch.side_effect = [[{"date": "2024-05-24T15:00:00", "close": 123.45}], None]
        mock_cache.return_value = True
        result = app.market_data.fetch_intraday_data(symbol)
        self.assertIsInstance(result, list)
        with patch('app.market_data.load_cached_data', return_value=[{"date": "2024-05-23T15:00:00", "close": 120.00}]):
