...
class TestRoutes(unittest.TestCase):
    def setUp(self):
        ...
        patcher_state = patch('api.routes.trading_state', {
    ...
    def test_cancel_mystrategy_orders_invalid_symbol(self):
        response = self.client.post('/cancel_mystrategy_orders', json={"symbol": "INVALID"})
    ...
    def test_get_trades(self):
        from api.routes import trading_state
        trading_state["trades"] = {"TESTSYM": [{"side": "1", "source": "my_strategy", "price": 100}]}
    ...
    def test_get_order_book_history(self):
        ...
        trading_state["order_book_history"] = {
            "TESTSYM": [{"time": "now", "snapshot": {"bids": [{"price": 100, "quantity": 10}], "asks": [{"price": 101, "quantity": 5}]}}]
        }
    ...
    def test_get_spread_history(self):
        from api.routes import trading_state
        trading_state["spread_history"] = {"TESTSYM": [{"time": "now", "mid": 100, "spread": 1}]}
    ...
    def test_get_liquidity_history(self):
        from api.routes import trading_state
        trading_state["liquidity_history"] = {"TESTSYM": [{"time": "now", "liquidity": 1000}]}
